...
| CMS editorial workflow | `public/admin/index.html` pins Decap CMS; `src/admin/config.yml` enables editorial workflow and collection editing, served through `src/pages/admin/config.yml.ts`. | Implemented as a shell. Production identity, MFA, roles, Git backend, preview, CSP, and access controls remain launch gates. |
